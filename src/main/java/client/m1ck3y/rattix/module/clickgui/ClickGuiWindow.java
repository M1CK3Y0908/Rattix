...
public class ClickGuiWindow {
    ...
    public static final List<TabPreset> AVAILABLE_PRESETS = Arrays.asList(
            new TabPreset("modules", "Modules", null),
            new TabPreset("console", "Console", null),
            new TabPreset("combat", "Combat", Category.COMBAT),
            new TabPreset("movement", "Movement", Category.MOVEMENT),
            new TabPreset("render", "Render & HUD", Category.RENDER),
            new TabPreset("player", "Player", Category.PLAYER),
            new TabPreset("world", "World & Misc", Category.WORLD),
            new TabPreset("theme", "Theme", Category.THEME)
    );
    ...
    private void drawModuleManagement(TabInfo tab, float x, float y, float w, float h, int mouseX, int mouseY) {
        ...
        List<Module> mods = tab.isModulesTab()
                ? RattixMod.instance.getModuleManager().getAlphabeticalModules()
                : RattixMod.instance.getModuleManager().getModulesByCategory(cat);
        ...
        Module selectedModule = tab.getSelectedModule();
    ...
    private void drawThemeFontManagement(TabInfo tab, float x, float y, float w, float h, float rightX, float sidebarW, int mouseX, int mouseY) {
        ...
        client.m1ck3y.rattix.module.impl.Font font = client.m1ck3y.rattix.module.impl.Font.getInstance();
        ...
        if (activeTab != null && (activeTab.isModulesTab() || activeTab.getCategory() != null)) {
            ...
            if (RenderUtil.isHovered(mouseX, mouseY, curX, bodyY, sidebarW, bodyH)) {
                if (cat == Category.THEME) {
                    ...
                    List<Module> mods = activeTab.isModulesTab()
                            ? RattixMod.instance.getModuleManager().getAlphabeticalModules()
                            : RattixMod.instance.getModuleManager().getModulesByCategory(cat);
                    float itemY = bodyY + 14 + activeTab.getModuleListScrollY();
            ...
            Module selectedModule = activeTab.getSelectedModule();
    ...
    public void executeCommand(TabInfo tab, String rawCmd) {
        ...
        if (cmd.equals("cls") || cmd.equals("clear")) {
            ...
            for (Module m : RattixMod.instance.getModuleManager().getModules()) {
            ...
            if (parts.length > 1) {
                String name = parts[1];
                Module m = RattixMod.instance.getModuleManager().getModuleByName(name);
                if (m != null) {
                    m.toggle();
